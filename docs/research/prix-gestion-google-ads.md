...
> État au 30 juillet 2026 — **P4 agent terminée, en attente du gate G4**
> Statut de la page : brouillon local non publié. Le statut d’indexation, le
> title, la meta et la date réelle restent à réconcilier par l’orchestrateur
> dans le registre central.
> Ce dossier remplace le contenu antérieur pour repartir d’un contrat de réponse
> vérifiable. Les quatre passes sont documentées ci-dessous ; aucune publication
> n’est déduite de leur exécution locale.
...
**Autorisé après G4, non encore validé.** Il intervient avant toute publication,
avec contrôle navigateur, données structurées, SEO, maillage, build, qualité
humaine et preuve publique après déploiement.

### H6. Vérité de livraison après P3

- aucune publication ;
- aucune indexation demandée ;
- aucun commit ;
- aucun push ;
- la reprise P2 a reçu le verdict orchestrateur `GO_PASSE_3_APRES_REPRISE` ;
- P3 est un candidat en attente de validation orchestrateur, pas un GO P4 ;
- aucune validation P4 ou transversale ;
- le timestamp du registre est un timestamp de création du brouillon et devra
  être réconcilié avec la publication réelle si le guide franchit tous les
  contrôles.
