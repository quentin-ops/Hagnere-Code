...
- Progression : signalement → qualification → délais → capacité → accès/données → recette/reporting → sortie.
- Artefact signature : console contractuelle « clause → question → preuve → responsable → acceptation » et mini-RACI.
- Rythme/voix : responsable d’exploitation pédagogique, qui arrête le chronomètre à chaque ambiguïté.
...
| --- | --- | --- | --- |
| Clause ambiguë | le contrat sait-il recevoir un incident ? | scénario fictif clairement qualifié | identifier les zones muettes |
| Cinq familles | qu’est-ce qui est réellement acheté ? | correctif, préventif, évolutif, support, exploitation | inclure/exclure |
| Cycle du ticket | où commence et finit le service ? | états et horodatages | rendre le flux auditable |
| Gravité et temps | quel impact, quel calendrier ? | matrice impact/contournement/données/horaires | écrire des objectifs interprétables |
| Capacité | comment le forfait se consomme-t-il ? | file, unité, arrondi, report, urgence, dépassement | simuler un mois |
| Gouvernance | qui décide et fournit quoi ? | mini-RACI | éviter la responsabilité implicite |
| Sécurité et données | quels accès, sauvegardes et sous-traitants ? | registre de preuves | adapter au risque |
| Livraison et recette | qu’est-ce qu’un ticket terminé ? | critère, test, déploiement, retour arrière | accepter ou rouvrir |
| Reporting | le contrat s’améliore-t-il ? | incidents, backlog, capacité, causes, risques, décisions | revue mensuelle |
| Réversibilité | peut-on quitter ? | exercice de sortie à blanc | signer, corriger ou refuser |
| Alternatives | faut-il vraiment une TMA ? | interne, ponctuel, capacité, lot projet, reprise | choisir le mode |
| CTA | quand un audit aide-t-il ? | bon/mauvais fit | agir seul ou cadrer |
...
| Décision | 2 | simulation de panne, chaîne de tickets, matrice et exercice de sortie |
| Pédagogie | 2 | délais, gravité, capacité et responsabilités traduits en questions concrètes |
| Profondeur | 2 | périmètre, SLA, RACI, sécurité, sauvegarde, recette et réversibilité |
...
| Style | 2 | ouverture contractuelle propre au sujet et progression opérationnelle |
| Conversion | 2 | exercice autonome, cas inadaptés et CTA vers un audit de périmètre |
| SEO/produit | 2 | métadonnées, canonical, maillage, Article, BreadcrumbList et OG dédiée |

Score final : **20/20 — publiable**.
...
Clôture parent : 20/20 après tests, build et rendu aux dix largeurs
```
...
- [x] temps de lecture recalculé à 20 minutes ;
- [x] score final supérieur ou égal à 17/20, axes obligatoires à 2 ;
- [x] contre-audit indépendant corrigé ;
- [x] batterie finale après ouverture à l’indexation ;
- [x] présence dans les artefacts SEO de production vérifiée, sans prétendre que Google a déjà indexé la page.
