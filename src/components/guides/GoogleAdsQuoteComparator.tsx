            ...
            <div key={field.key}>
              ...
              <span className="relative mt-1.5 block">
                <input
                  ...
                  aria-invalid={error !== null}
                  onChange={(event) => update(field.key, event.target.value)}
                  className={`w-full rounded-lg border bg-white px-3 py-2.5 pr-14 text-sm tabular-nums text-zinc-950 outline-none transition focus:ring-2 dark:bg-zinc-950 dark:text-white ${
                    error
                      ? "border-red-500 focus:border-red-600 focus:ring-red-200 dark:border-red-700 dark:focus:ring-red-950"
                      : "border-zinc-300 focus:border-indigo-500 focus:ring-indigo-200 dark:border-zinc-700 dark:focus:ring-indigo-950"
                  }`}
                />
                <span className="pointer-events-none absolute inset-y-0 right-3 flex items-center text-[10px] font-medium text-zinc-500">
                  € HT
                </span>
              </span>
              {error && (
