    ...
    >
      <style>{`
        ...
        @media print {
          ...
          [data-guide-premium-faq="true"] [data-slot="accordion-trigger"] {
            break-after: avoid;
          }
        }
      `}</style>
      <div className="container mx-auto px-4 max-w-3xl">
        ...
        <h2 className="text-center font-[family-name:var(--font-playfair)] text-[28px] leading-[1.15] sm:text-[36px] sm:leading-[1.1] md:text-[42px] md:leading-[1.08] font-bold tracking-tight text-zinc-950 dark:text-white mb-4">
          {titleStart && titleEm ? (
            <>
              {titleStart}{" "}
        ...
        >
          {items.map((item, index) => {
            ...
            return (
              ...
              >
                <AccordionTrigger className="hover:no-underline py-5 text-left gap-4 [&>svg]:hidden [&[data-state=open]_.faq-plus]:hidden [&[data-state=closed]_.faq-minus]:hidden">
                  <div className="flex items-center gap-4 min-w-0 flex-1">
                    <span className="text-[11px] font-mono font-medium text-zinc-600 dark:text-zinc-400 tabular-nums tracking-wider shrink-0">
                      {number}
                    </span>
                  ...
                  <span
                    aria-hidden="true"
                    className="faq-plus inline-flex items-center justify-center size-7 rounded-full text-zinc-400 shrink-0 group-hover:text-zinc-700 transition-colors print:hidden"
                  >
                    <Plus className="size-4" />
                  </span>
                  <span
                    aria-hidden="true"
                    className="faq-minus inline-flex items-center justify-center size-7 rounded-full bg-zinc-950 text-white shrink-0 print:hidden"
                  >
                ...
                <AccordionContent
                  forceMount
                  className="text-zinc-600 dark:text-white text-sm leading-[1.7] pb-5 pl-[44px] pr-2"
                >
                  {item.answer}
                </AccordionContent>
              </AccordionItem>
